...
//        LoggerLog(prefix + " ----fields end-----");
    ...
    public static void searchStringInObjectFields(Object obj, String prefix,String toSearch){
        searchStringInObjectFields(obj, prefix,new HashSet<Object>(),toSearch);
    }

    public static void searchStringInObjectFields(Object obj, String prefix, HashSet<Object> filter,String toSearch) {
        prefix += "\n";
        if (obj==null){return;}
        String typeName = obj.getClass().getTypeName();
        String typeNameLower = typeName.toLowerCase();
            ...
            for (Field f:obj.getClass().getDeclaredFields()){
                try {
                    ...
                    if (!typeNameLower.startsWith("java")
                        ...
                        String check = f.getName() + "     " + fobj.getClass() + "      " + typeName;
                        if (check.contains(toSearch)){
                            LoggerLog(prefix + "     fieldName:" + check);
                        }
                        if (filter.contains(fobj)){
//                        LoggerLog("--contained:"+fobj);
                            continue;
                        ...
                        filter.add(fobj);
                        if (showDeepFields){
                            searchStringInObjectFields(fobj, prefix + fobj.getClass() +"(" + f.getName() + ")     ", filter,toSearch);
                        }
                        if (fobj.getClass().isArray()){
                            int len = Array.getLength(fobj);
//                            LoggerLog(prefix + "array length:" + len);
                            for (int i=0;i<len;i++){
                                ...
                                filter.add(o);
                                if (showDeepFields){
                                    searchStringInObjectFields(o, prefix + o.getClass().toString() + "[]     ", filter,toSearch);
                                }
                            }
                        }
                    }
                    else {
                        showField_search(obj,f,prefix,typeName,toSearch);
                    }
                    if (fobj instanceof Map){
//                        LoggerLog(prefix + "map size:" + ((Map)fobj).size());
                        for (Map.Entry<?,?> entry:((Map<?,?>)fobj).entrySet()){
                            String check = "key: " + entry.getKey() + "      value:" + entry.getValue();
                            if (check.contains(toSearch)){
                                LoggerLog(prefix + check);
                            }
                            if (!filter.contains(entry.getKey())){
                                filter.add(entry.getKey());
//                                LoggerLog(prefix + "keyField:");
                                if (showDeepFields){
                                    searchStringInObjectFields(entry.getKey(), prefix + "[key]" + entry.getKey().getClass() + "  ", filter,toSearch);
                                }
                            }
                            if (!filter.contains(entry.getValue())){
                                filter.add(entry.getValue());
//                                LoggerLog(prefix + "valueField:");
                                if (showDeepFields){
                                    searchStringInObjectFields(entry.getKey(), prefix + "[value]" + entry.getValue().getClass() + "  ", filter,toSearch);
                                }
                            }
                        }
                    }
...
//                        LoggerLog(prefix + "collection size:" + ((Collection)fobj).size());
                        for (Object o:(Collection)fobj){
                            ...
                            if (filter.contains(o)){continue;}
                            filter.add(o);
                            searchStringInObjectFields(o,prefix + "[collection]" + o.getClass().toString() + "  ",filter,toSearch);
                        }
                    }
                }catch (Exception e){
                    LoggerLog(prefix + "     cannotAccess(" + f.getType().toString() + ")");
            ...
            while (!Objects.equals(superClass, Object.class)
                ...
                prefix +=  "[superClass]" + superClass.getTypeName() + "  ";
//                LoggerLog(prefix + "superClass:" + superClass.getTypeName());
                for (Field f:superClass.getDeclaredFields()){
                    try {
                        typeName = f.getType().getTypeName();
                        ...
                        if (!typeNameLower.startsWith("java")
                            ...
                            String check = "     fieldName:" + f.getName() + "     " + f.getType() + "      " + typeName;
                            if (check.contains(toSearch)){
                                LoggerLog(prefix + "     fieldName:" + f.getName() + "     " + fobj + "      " + typeName);
                            }
                            if (filter.contains(fobj)){
...
//                                LoggerLog(prefix + "valueField:");
            ...
            if (superClass != null && superClass.isEnum()){
                String check = "     enumFieldName:" + obj.getClass() + "     " + obj + "      " + typeName;
                if (check.contains(toSearch)){
                    LoggerLog(prefix + check);
                }
            }else {
                String check = "     fieldName:" + obj.getClass() + "     " + obj + "      " + typeName;
                if (check.contains(toSearch)){
                    LoggerLog(prefix + check);
                }
            }
...
//        LoggerLog(prefix + " ----fields end-----");
    ...
    public static void showField_search(Object obj,Field f,String prefix,String typeName,String toSearch){
            String check = "     fieldName:" + f.getName() + "     " + f.getType() + "      " + typeName;
            if (!check.contains(toSearch)){return;}
        if (!Modifier.isStatic(f.getModifiers())){
            if (f.getType().equals(long.class)){
                LoggerLog(prefix + "     fieldName:" + f.getName() + "     " + XposedHelpers.getLongField(obj,f.getName()) + "      " + typeName);
            }else if (f.getType().equals(int.class)){
